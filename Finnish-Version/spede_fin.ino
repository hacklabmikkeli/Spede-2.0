...
if (step_finalScores == true) {
  if (showText == 1) {
    if ( (millis()- currentTimeFinalScores) >= onDurationFinalScores )  {   
      ...
      if (scorePlayer1 > scorePlayer2) {
        P.displayZoneText(0, "Voittaja", PA_CENTER, 0, 0, PA_PRINT, PA_NO_EFFECT);
        } else {
        P.displayZoneText(1, "Voittaja", PA_CENTER, 0, 0, PA_PRINT, PA_NO_EFFECT);
      }
      P.displayAnimate();  
    }
  }
...
void showScores() {
  ...
  else if (wrongButtonPlayer1 == true)  {
  ...
  else if (wrongButtonPlayer2 == true)  {
...
void startupAnimation(void)  {
    ...
    if (P.getZoneStatus(0) && P.getZoneStatus(1)) {
      P.displayZoneText(0, "Pelaaja 1, valmiina, ALOITA!", PA_CENTER, P.getSpeed(), P.getPause(), PA_SCROLL_LEFT, PA_SCROLL_LEFT);
      P.displayZoneText(1, "Pelaaja 2, valmiina, ALOITA!", PA_CENTER, P.getSpeed(), P.getPause(), PA_SCROLL_LEFT, PA_SCROLL_LEFT);
      P.displayReset(0);
      P.displayReset(1);
      P.synchZoneStart();
    }
